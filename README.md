...
- A 20 MB file/ZIP budget keeps replies manageable. Invalid, non-public, and oversized files are skipped and reported. PDFs must parse; other supported files receive signature checks, not full media decoding. Size limits are enforced while streaming. If retrieval reaches its ten-minute deadline, the reply includes completed files and reports that the remaining documents were not checked.
