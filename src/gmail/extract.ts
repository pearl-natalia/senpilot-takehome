...
export async function extractRequest(email: Pick<IncomingEmail, 'subject' | 'text'>): Promise<ExtractionResult> {
  const body = email.text.split(/^On .+wrote:|^-{2,}\s*(?:Original Message|Forwarded message)/im)[0]!.split('\n').filter(line => !/^\s*>/.test(line)).join('\n');
  ...
  if (extracted.status !== 'ready' && !issues.length) issues.push('The request is ambiguous. Please clearly state one matter number and one document type.');
